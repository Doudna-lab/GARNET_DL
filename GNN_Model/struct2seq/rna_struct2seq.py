...
class RNAStruct2Seq(nn.Module):
    def __init__(self, num_letters, node_features, edge_features, dist_map,
                 hidden_dim, num_encoder_layers=3, num_decoder_layers=3,
                 k_neighbors=30, augment_eps=0.,
        ...
        self.features = RNAFeatures(
            node_features, edge_features, dist_map, top_k=k_neighbors,
    ...
    def _autoregressive_mask(self, E_idx):
        ...
        ii = torch.arange(N_nodes, device=device)
        ii = ii.view((1, -1, 1))
        mask = E_idx - ii < 0
    ...
    def forward(self, X):
        ...
        S = X
        L = torch.ones(X.size(0), device=X.device) * X.size(1)
        mask = torch.ones_like(X)
        V, E, E_idx = self.features(X)
        h_V = self.W_v(V)
        h_E = self.W_e(E)
