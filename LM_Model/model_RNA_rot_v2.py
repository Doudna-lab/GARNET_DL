...
#        print(f"using fused AdamW: {use_fused}")
    ...
    @torch.no_grad()
    def sequence_probability_masked(self, idx, mask):
        """
        Calculate the log probability of a given sequence idx (LongTensor of shape (b,t))
        being generated by the model. Handles batches of any size. Also takes in masked
        positions, and expands these to include +/- 1 token (to account for overlapping triples).
        """
        b, t = idx.size()
        log_probs = torch.zeros(b, device=idx.device)  # Initialize log probabilities for each sequence in the batch

        #Expand mask positions to include +/- 1 token, taking care of sequence boundaries.
        expanded_mask = set(mask)
        for pos in mask:
            if pos > 0:
                expanded_mask.add(pos - 1)
            if pos < t - 1:
                expanded_mask.add(pos + 1)

        expanded_mask = sorted(list(expanded_mask))  # Convert into a sorted list for consistency
        print(expanded_mask)

        # Shift expanded_mask from 1-indexed to 0-indexed to align with idx.
        expanded_mask = [pos-1 for pos in expanded_mask]
        #print(expanded_mask)
        ...
        for i in range(1, t):  # Start from 1 since the first token has no preceding context
            ...
            # Check if the current position is in the expanded mask
            if i not in expanded_mask:
                # Apply masking logic
                log_token_probs = log_probs_next_token.gather(1, next_tokens).squeeze(1)
            else:
                # If not in the mask (or expanded mask), set log probability to zero for this position
    ...
    @torch.no_grad()
    def sequence_probability_sweep_masked(self, idx, mask):
        """
        Calculate the log probability of a given sequence idx (LongTensor of shape (b,t))
        being generated by the model. Handles batches of any size. Also takes in a boolean mask
        of dimensions b, t which includes +/- 1 token positions (to account for overlapping triples).
        """
        b, t = idx.size()
        log_probs = torch.zeros(b, device=idx.device)  # Initialize log probabilities for each sequence in the batch

        for i in range(1, t):  # Start from 1 since the first token has no preceding context
            # Initialize log_token_probs for this iteration
            log_token_probs = torch.zeros(b, device=idx.device)
            ...
            # Determine which positions to mask based on a binary mask matrix. 0-indexed
            is_masked = mask[:, i]

            # For non-masked positions, calculate the log probability
            non_masked_indices = ~is_masked
            if non_masked_indices.any():
                log_token_probs[non_masked_indices] = log_probs_next_token[non_masked_indices].gather(1, next_tokens[non_masked_indices]).squeeze(1)
