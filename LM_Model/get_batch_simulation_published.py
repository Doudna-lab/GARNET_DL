...
for batch_idx in range(num_batches):
    ...
    # Also check for correctly aligned x and y across the entire context window.
    #row_correct_matches = torch.where(torch.all(x[:, 1:] == y[:, :-1], dim=1))[0]  # Indices where rows are correctly equal
    row_correct_matches = torch.where(torch.all(x[:, 1:101] == y[:, :100], dim=1))[0]  # Indices where rows are correctly equal, thru n tokens.
...
print(f"Out of {num_batches} batches, there were {total_correct_matches} total correctly matching rows x[b,1:] = y[b,:-1].")
