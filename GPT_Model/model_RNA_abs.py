...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_emb = nn.Embedding(config.block_size, self.n_embd)  # Use for absolute positional embedding

    def forward(self, x, mask=None):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality (n_embd)

        # Apply absolute positional embeddings
...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding=True):
        ...
        if non_embedding:
            ...
            # Subtract parameters of absolute positional embeddings in each CausalSelfAttention instance
            for block in self.transformer.h:
                if hasattr(block.attn, 'pos_emb'):
                    n_params -= block.attn.pos_emb.weight.numel()
