...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.flash:
            ...
            #if mask is not None:
                # Allow self-attention for padding.
                #diag_mask = torch.eye(T, T, device=mask.device)
                #diag_mask = ~diag_mask.bool()
                #mask = mask.repeat(1, 1, T, 1)
                #mask |= diag_mask.unsqueeze(0).unsqueeze(1)
                #att = att.masked_fill(mask, float(-1e3))
...
class GPT(nn.Module):
    ...
    def __init__(self, config, pad_index=None):
        ...
        # Incorporate rotary embedding here, and potentially remove positional embedding.
        self.rotary_emb = self.build_rotary_embedding(config.n_embd // config.n_head)
    ...
    def build_rotary_embedding(self, head_dim, block_size=384):
        pos = torch.arange(0, block_size, dtype=torch.float)
        div_term = torch.exp(torch.arange(0, head_dim, 2).float() * (-math.log(10000.0) / head_dim))
        rotary_embedding = torch.zeros((block_size, head_dim))
        rotary_embedding[:, 0::2] = torch.sin(pos[:, None] * div_term)
        rotary_embedding[:, 1::2] = torch.cos(pos[:, None] * div_term)
        return rotary_embedding

    def apply_rotary_embeddings(self, x, block_size):
        dim = x.shape[-1]
        rotary_emb = self.rotary_emb[:block_size, :].to(x.device)
    ...
    def forward(self, idx, pad_index=None, targets=None):
        ...
        x = tok_emb.view(b, t, self.n_head, self.n_embd // self.n_head).transpose(1, 2) # (b, nh, t, hs)
        x = self.apply_rotary_embeddings(x, t)
        x = x.transpose(1, 2).contiguous().view(b, t, self.n_embd) # re-assemble all head outputs side by side
